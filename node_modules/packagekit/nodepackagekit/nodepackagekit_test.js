...
jqUnit.test ("Testing PackageKit Bridge", function() {
    ...
    jqUnit.assertNotEquals ("Did not find tuxguitar in /*!
GPII Node.js PackageKit Bridge
...
jqUnit.test ("Testing PackageKit Bridge", function() {
    ...
    if (pkgs[0].data.indexOf ('installed') != -1) {
    ...
    var availablePkgs = packagekit.getPackages ('~installed');
    var availableIndex = -1;
    for (var i = 0; i < availablePkgs.length; i++) {
      if (availablePkgs[i].name === tuxguitarPkg.name) {
        availableIndex = i;
        break;
      }
    ...
    if (tuxguitarPkg.data.indexOf ('installed') != -1) {
    ...
    if (pkgs.length === 2 &&
        (pkgs[0].version === oldEmacspeak && pkg[1].version === newEmacspeak &&
...
available packages", -1, availableIndex);
